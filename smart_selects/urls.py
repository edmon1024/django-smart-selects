...
urlpatterns = [
    ...
    url(
        r"^filter/(?P<app>[\w\-]+)/(?P<model>[\w\-]+)/(?P<child_app>[\w\-]+)/(?P<child_model>[\w\-]+)/(?P<child_field>[\w\-]+)/(?P<field>[\w\-]+)/(?P<foreign_key_app_name>[\w\-]+)/(?P<foreign_key_model_name>[\w\-]+)/(?P<foreign_key_field_name>[\w\-]+)/(?P<value>[\w\-,]+)/$",  # noqa: E501
        views.filterchain,
        name="chained_filter",
    ),
    url(
